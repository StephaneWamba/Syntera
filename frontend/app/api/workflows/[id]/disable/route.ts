/**
 * Next.js API Route - Disable Workflow
 */
...
export async function POST(_request: NextRequest,
  ...
  try {
    ...
    const response = await fetch(`${AGENT_SERVICE_URL}/api/workflows/${id}/disable`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to disable workflow' }, { status: response.status })
    }
